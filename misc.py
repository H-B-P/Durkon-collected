...
def de_feat(model, boringValue=1): #Not expanded to interxes
...
def caricature_model(model, mult=1, frac=0.5, boringValue=1): #Not expanded to interxes
...
def get_importance_of_this_cont_col(df, model, col, boringValue=1):
 effects = get_effect_of_this_cont_col(df, model, col)
 effects = effects/effects.mean()
 effects = (effects-boringValue).apply(abs)
 return effects.mean()

def get_importance_of_this_cat_col(df, model, col, boringValue=1):
 effects = get_effect_of_this_cat_col(df, model, col)
 effects = effects/effects.mean()
 effects = (effects-boringValue).apply(abs)
 return effects.mean()

def get_importance_of_this_contcont(df, model, cols, boringValue=1):
 effects = get_effect_of_this_contcont(df, model, cols)
 effects = effects/effects.mean()
 effects = (effects-boringValue).apply(abs)
 return effects.mean()

def get_importance_of_this_catcont(df, model, cols, boringValue=1):
 effects = get_effect_of_this_catcont(df, model, cols)
 effects = effects/effects.mean()
 effects = (effects-boringValue).apply(abs)
 return effects.mean()

def get_importance_of_this_catcat(df, model, cols, boringValue=1):
