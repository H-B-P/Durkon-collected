...
def prep_model(df, resp, cats, conts, catMinPrev=0.01, contTargetPts=5, edge=0.01, defaultValue=1, weightCol=None):
 ...
 model={"BASE_VALUE":df[resp].mean(), "mechanism":"mult"}
...
def get_contcont_feat(df, cont1, cont2, contTargetPts1=3, contTargetPts2=3, edge1=0.05, edge2=0.05, defaultValue=1, weightCol=None):
 ...
 feat1 = get_cont_col(df, cont1, contTargetPts1, edge1, defaultValue, weightCol)
 feat2 = get_cont_col(df, cont2, contTargetPts2, edge2, defaultValue, weightCol)
 
 for i in range(len(feat1)):
  feat1[i][1]=feat2.copy()
...
def get_catcont_feat(df, cat, cont, catMinPrev=0.1, contTargetPts=3, edge=0.05, defaultValue=1, weightCol=None):
 ...
 feat1 = get_cat_col(df, cat1, catMinPrev, defaultValue, weightCol)
 feat2 = get_cont_col(df, cont2, contTargetPts, edge, defaultValue, weightCol)
 
 feat1["OTHER"]=feat2.copy()
 
 for u in feat1["uniques"]:
  feat1["uniques"][u]=feat2.copy()
...
def get_catcat_feat(df, cat1, cat2, catMinPrev1=0.1, catMinPrev2=0.1, defaultValue=1, weightCol=None):
 ...
 feat1 = get_cat_col(df, cat1, catMinPrev1, defaultValue, weightCol)
 feat2 = get_cat_col(df, cat2, catMinPrev2, defaultValue, weightCol)
